...
Shader"Lyf/Environment/NormalWater"
{
    Properties
    {
        ...
        _WaterAlpha("Alpha",Range(0,10))=1
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            sampler2D _CameraDepthTexture,_RefractionTex;
            ...
            inline void GetDepthFlag(float4 uv, float4 nuv,float3 w,out float relDepth,out float flag)
            {
                float depth=Linear01Depth(UNITY_SAMPLE_DEPTH(tex2Dproj(_CameraDepthTexture,UNITY_PROJ_COORD(nuv))));
                float planedepth=WorldPostionToLinearDepth(w);
                float rd=Linear01Depth(UNITY_SAMPLE_DEPTH(tex2Dproj(_CameraDepthTexture, UNITY_PROJ_COORD(uv))));
                relDepth=(depth-planedepth);
                flag=step(-0.01,relDepth);
                relDepth=rd-planedepth;
                relDepth=smoothstep(0.001,0.055,relDepth);
                float temp=relDepth;
                relDepth=(depth-planedepth);
                relDepth=smoothstep(0.001,0.055,relDepth);
                relDepth*=flag;
            ...
            fixed4 frag(v2f i):SV_Target
            {
                ...
                float4 uv=originPos;
                uv.xy+=ofs;
                GetDepthFlag(originPos,uv,wpos,rD,flag);
