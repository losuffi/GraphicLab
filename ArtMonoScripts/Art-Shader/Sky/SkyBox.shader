...
Properties {
    ...
    _SkyTex ("Sky Tex",2D)=""{}
    _TexWeight("TexWeight",Range(0,1))=0.1
    _GroundColor ("Ground", Color) = (.369, .349, .341, 1)

    _Exposure("Exposure", Range(0, 8)) = 1.3
    _DepthPow("PowCoff",Float)=1
    _OutColorPow("OutPow",Float)=4
...
SubShader {
    ...
    Pass {
        ...
        sampler2D _SkyTex;
        float4 _SkyTex_ST;
        float _TexWeight,_DepthPow,_OutColorPow;


    #if defined(UNITY_COLORSPACE_GAMMA)
        float _Gamma;
        #define COLOR_2_GAMMA(color) pow(color,1.0/_Gamma)
        #define COLOR_2_LINEAR(color) color
        #define LINEAR_2_OUTPUT(color) color
    #else
        float _Gamma;
        // HACK: to get gfx-tests in Gamma mode to agree until UNITY_ACTIVE_COLORSPACE_IS_GAMMA is working properly
        #define COLOR_2_GAMMA(color) ((unity_ColorSpaceDouble.r>2.0) ? pow(color,1.0/_Gamma) : color)
        #define COLOR_2_LINEAR(color) color
...
//              for(int i=0; i<int(kSamples); i++)
                {
                    ...
                    frontColor += attenuate * (pow(depth * scaledLength,_DepthPow) );
                ...
                float3 dds=1.0 / pow(kScatteringWavelength, _OutColorPow);
                cIn = frontColor * (dds * kKrESun);
...
//              for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
        ...
        // if y < 0 [eyeRay.y > 0] - sky
            ...
            half3 ray = float3(1,0,0);
        #endif

            // if we did precalculate color in vprog: just do lerp between them
            float3 x_Space=normalize(float3(ray.x,0,ray.z));
            float Ux=acos(dot(float3(1,0,0),x_Space));
            float axis=dot(float3(0,0,1),x_Space);
            float flag=step(0,axis);
            Ux=(2*3.1415926-Ux)*flag+(1-flag)*Ux;
            float2 x_y=float2(cos(Ux),sin(Ux));
            //float Ux=(dot(float3(-1,0,0),x_Space))*0.5+0.5;
            //float Uy=(dot(float3(0,-1,0),normalize(ray)))*0.5+0.5;
            //Ux=smoothstep(0,2*3.1415926,Ux);
            //return Ux;
            axis=dot(float3(1,0,0),normalize(ray));
            flag=step(0,axis);
            float Uy=acos(dot(float3(0,1,0),normalize(ray)));
            Uy=(2*3.1415926-Uy)*flag+(1-flag)*Uy;
            Uy=sin(Uy);
            Uy=abs(Uy)*0.5;
            x_y=normalize(x_y)*Uy;
            x_y+=0.5;
            //return float4(x_y,0,1);
           // return Uy;
            float4 s=tex2D(_SkyTex,half4(x_y.x*_SkyTex_ST.x+_SkyTex_ST.z,x_y.y*_SkyTex_ST.y+_SkyTex_ST.w,0,0));
            //float3 s=tex2D(_SkyTex,IN.uv).rgb;
            col =lerp(IN.skyColor,s.rgb,_TexWeight*s.a);
            
            col = lerp(col, IN.groundColor, saturate(y));
